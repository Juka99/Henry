$(document).ready(function(){
    ...
    // Dodavanje producta na dugme

    $('#insertProductButton').on('click',function(){
        ...
        if(productValue == ''){
            ...
            document.getElementById('errorInsertingPar').style.display = 'block';
            ...
            document.getElementById('errorInsertingPar').style.display = 'none';

            var optionalWeight = document.getElementById('weightOfProduct').value;

            document.getElementById('successInsertingPar').style.display = 'block';

            setTimeout(function(){

                document.getElementById('successInsertingPar').style.display = 'none';
                    ...
                    for(let el of data){
                        ...
                        if(el.id == productValue){
                            ...
                            if(localStorage){
                                ...
                                alert('Your browser does not support LocalStorage, please stop using Internet Explorer');                             
    ...
    // Pretraga proizvoda preko input polja
    ...
    $('#nameProductSearch').on('keyup', function(){
        ...
        $.ajax({
            ...
            fillTheSelect(filtriraniPostovi);
    ...
    // Ispis limita za trenutni plan ako postoji

    printYourNewPlan();

    // Kalkulator za racunanje kalorija

    $("#successCalcButton").on('click', calculatorCheck);
...
// Ispis limita za trenutni plan ako postoji
    ...
    function printYourNewPlan(){

        if(localStorage){

            if(localStorage.getItem('personalStatsForHenry')){
        
                var pullPersonalStatsForPrint = JSON.parse(localStorage.getItem('personalStatsForHenry'));
        
                document.querySelector('.specialHideBlock').style.display = 'block';
        
                document.getElementById('yourPlanCalorieValue').innerHTML = pullPersonalStatsForPrint.calorieLimit;
        
                document.getElementById('yourPlanProteinValue').innerHTML = pullPersonalStatsForPrint.proteinLimit;
        
                document.getElementById('yourPlanCarbsValue').innerHTML = pullPersonalStatsForPrint.carbsPerDay;
        
                document.getElementById('yourPlanFatsValue').innerHTML = pullPersonalStatsForPrint.fatsPerDay;
        
            }
        
            else{
        
                document.querySelector('.specialHideBlock').style.display = 'none';
    ...
    // Funckija za prikaz bmr, proteina, hidrata i masti na osnovu popunjenog kalkulatora

    function updatePersonalStatsLocal(){

        var personalStatsLocal = JSON.parse(localStorage.getItem('personalStats'));

        console.log(personalStatsLocal)

        var bmrStatsValue = 10 * personalStatsLocal.weightInKg + 6.25 * personalStatsLocal.heightInCm - 5 * personalStatsLocal.age;

        if(personalStatsLocal.gender == 'male'){

            bmrStatsValue += 5

        }

        if(personalStatsLocal.gender == 'female'){

            bmrStatsValue -= 161;

        }

        document.getElementById('userBmrSpan').innerHTML = Math.round(bmrStatsValue);

        var yourGoalString, caloriesPerDay = 0;

        if(personalStatsLocal.goal == 'loseWeight'){

            yourGoalString = "losing";

            caloriesPerDay = bmrStatsValue - 500;

        }

        if(personalStatsLocal.goal == 'gainWeight'){

            yourGoalString = "gaining";

            caloriesPerDay = bmrStatsValue + 500;

        }

        document.getElementById('planSpanText').innerHTML = yourGoalString;

        document.getElementById('caloriesPlanCalc').innerHTML = Math.round(caloriesPerDay);

        var proteinPerDay = Math.round(40/100 * caloriesPerDay / 4);
        
        var carbsPerDay = Math.round(35/100 * caloriesPerDay / 4);

        var fatsPerDay = Math.round(25/100 * caloriesPerDay / 9);

        document.getElementById('proteinPlanCalc').innerHTML = proteinPerDay;

        document.getElementById('carbsPlanCalc').innerHTML = carbsPerDay;

        document.getElementById('fatsPlanCalc').innerHTML = fatsPerDay;

        var statsForHenry = {

            "calorieLimit" : Math.round(caloriesPerDay),
            "proteinLimit" : proteinPerDay,
            "carbsPerDay" : carbsPerDay,
            "fatsPerDay" : fatsPerDay

        }
        
        localStorage.setItem('personalStatsForHenry', JSON.stringify(statsForHenry));

    }

    // Funckija za calculator kalorija

    function calculatorCheck(){

        var calcErrors = [];

        // Hvatanje vrednosti polja iz kalkulatora

        var genderValue = document.getElementById('chooseGender').value;

        var weightValue = document.getElementById('kgSpecialField').value;

        var heightValue = document.getElementById('cmSpecialField').value;

        var ageValue = document.getElementById('ageSpecialField').value;

        var goalValue = document.getElementById('goalChoice').value;

        console.log(genderValue, weightValue, heightValue, ageValue, goalValue)

        // Regex za polja i provera za greske u dropdown

        if(genderValue == 0){

            calcErrors.push('error');
        ...
        var regexWeight = /^[1-9][0-9]{1,2}$/;

        var regexHeight = /^[12][\d]{2}$/;

        var regexAge = /^[1-9][\d]{1}$/;

        function checkRegex(field, regex, identificator){
        ...
        checkRegex(weightValue, regexWeight, 'kgError');

        checkRegex(heightValue, regexHeight, 'cmError');

        checkRegex(ageValue, regexAge, 'ageError');
        ...
        if(weightValue > 300 || weightValue < 45){

            calcErrors.push('error');

            document.getElementById("kgError").style.display = 'block';

        }

        else{

            document.getElementById("kgError").style.display = 'none';
        ...
        if(calcErrors.length == 0){
            ...
            localStorage.setItem("personalStats", JSON.stringify(yourStats));

            document.querySelector('.hideShowFinalCalcInfo').style.display = 'block';

            updatePersonalStatsLocal();

        }

        else{

            document.querySelector('.hideShowFinalCalcInfo').style.display = 'none';
    ...
    if(message.includes("nutrients info") || message.includes("nutrient info") || message.includes("info nutrients") || message.includes("nutrijent info")){
        ...
        if(localStorage){
            ...
            if(localStorage.getItem('products') && localStorage.getItem('products').length != 2){
                ...
                // Provera da li ima personal stats za Henryija

                if(localStorage.getItem('personalStatsForHenry')){

                    var henryUsesThisDataNow = JSON.parse(localStorage.getItem('personalStatsForHenry'));

                    // Recenice za kalorije

                    if(caloriesSumInfo > 0 && caloriesSumInfo <= 0.5 * henryUsesThisDataNow.calorieLimit){

                        nutrientsInfoSentence += `,You still have room for more calories`;
    
                    }

                    else if(caloriesSumInfo > 0.5 * henryUsesThisDataNow.calorieLimit && caloriesSumInfo <= 0.75 * henryUsesThisDataNow.calorieLimit){

                        nutrientsInfoSentence += `, Your calorie intake is good,`;

                    }

                    else if(caloriesSumInfo > 0.75 * henryUsesThisDataNow.calorieLimit && caloriesSumInfo <= 0.9 * henryUsesThisDataNow.calorieLimit){

                        nutrientsInfoSentence += `, You are getting close to completing your calorie intake,`;

                    }

                    else if(caloriesSumInfo > 0.9 * henryUsesThisDataNow.calorieLimit && caloriesSumInfo < henryUsesThisDataNow.calorieLimit){

                        nutrientsInfoSentence += `, You are very close to your calorie limit,`;

                    }
    
                    else{
    
                        nutrientsInfoSentence += `, You consumed more calories then you should`;
    
                    }

                    // Recenice za proteine

                    if(proteinSumInfo > 0 && proteinSumInfo <= 0.5 * henryUsesThisDataNow.proteinLimit){

                        nutrientsInfoSentence += `, your protein intake is still pretty low, `;

                    }

                    else if(proteinSumInfo > 0.5 * henryUsesThisDataNow.proteinLimit && proteinSumInfo <= 0.75 * henryUsesThisDataNow.proteinLimit){

                        nutrientsInfoSentence += `, your protein intake is very good`;

                    }

                    else if(proteinSumInfo > 0.75 * henryUsesThisDataNow.proteinLimit && proteinSumInfo < henryUsesThisDataNow.proteinLimit){

                        nutrientsInfoSentence += `, you are very close to completing your protein intake,`;

                    }

                    else{

                        nutrientsInfoSentence += `, you consumed more protein then the recommended amount, but that is not bad,`;

                    }

                    // Recenice za carbs

                    if(carbsSumInfo > 0 && carbsSumInfo <= 0.25 * henryUsesThisDataNow.carbsPerDay){

                        nutrientsInfoSentence += ` I recommend you consume more carbs`;

                    }

                    else if(carbsSumInfo > 0.25 * henryUsesThisDataNow.carbsPerDay && carbsSumInfo <= 0.5 * henryUsesThisDataNow.carbsPerDay){

                        nutrientsInfoSentence += `, your carbs intake is still a little low`;

                    }

                    else if(carbsSumInfo > 0.5 * henryUsesThisDataNow.carbsPerDay && carbsSumInfo <= 0.75 * henryUsesThisDataNow.carbsPerDay){

                        nutrientsInfoSentence += `, your carbs intake is good`;

                    }

                    else if(carbsSumInfo > 0.75 * henryUsesThisDataNow.carbsPerDay && carbsSumInfo <= henryUsesThisDataNow.carbsPerDay){

                        nutrientsInfoSentence += `, you are very close to completing your carbs intake`;
                    ...
                    if(fatsSumInfo > 0 && fatsSumInfo <= 0.25 * henryUsesThisDataNow.fatsPerDay){

                        nutrientsInfoSentence += `, your fats intake is still low`;

                    }

                    else if(fatsSumInfo > 0.25 * henryUsesThisDataNow.fatsPerDay && fatsSumInfo <= 0.5 * henryUsesThisDataNow.fatsPerDay){

                        nutrientsInfoSentence += `, your fats intake is not bad`;

                    }

                    else if(fatsSumInfo > 0.5 * henryUsesThisDataNow.fatsPerDay && fatsSumInfo <= 0.75 * henryUsesThisDataNow.fatsPerDay){

                        nutrientsInfoSentence += `, your fats intake is good`;

                    }

                    else if(fatsSumInfo > 0.75 * henryUsesThisDataNow.fatsPerDay && fatsSumInfo <= henryUsesThisDataNow.fatsPerDay){

                        nutrientsInfoSentence += `, you are very close to completing your fats intake`;
                    ...
                    else if(proteinSumInfo > 100 && proteinSumInfo < 150){

                        nutrientsInfoSentence += `, your intake is pretty good,`;
                    ...
                    if(carbsSumInfo > 0 && carbsSumInfo < 100){
                    ...
                    if(fatsSumInfo > 0 && fatsSumInfo < 50){
        ...
        if(localStorage){
            ...
            if(localStorage.getItem('products') && localStorage.getItem('products').length != 2){
                ...
                if(caloriesSum == 0){
                    ...
                    calculateNutrientsSentence += `you consumed, ${Math.round(caloriesSum)} calories,`;

                    if(proteinSum > 1000){

                        calculateNutrientsSentence += `, more then 1000 grams of protein,`;

                    }

                    else{

                        calculateNutrientsSentence += `${Math.round(proteinSum)} grams of protein,`;

                    }

                    if(carbsSum > 1000){

                        calculateNutrientsSentence += `, more then 1000 grams of carbs,`;

                    }

                    else{

                        calculateNutrientsSentence += `${Math.round(carbsSum)} grams of carbs,`;

                    }

                    if(fatsSum > 1000){

                        calculateNutrientsSentence += `, more then 1000 grams of fats,`;

                    }

                    else{

                        calculateNutrientsSentence += `and ${Math.round(fatsSum)} grams of fats,`;
